...
| `forbidden` | 403 | 놀이동산이 아직 해금되지 않음 (카페 미완료) | 재시도 금지. 카페로 안내 |
...
| `dialogue_stage_locked` | 409 | 아직 도달하지 않은 카페·놀이동산 단계의 대화 시작 요청 | 방문 조회 후 열린 단계로 이동 |
| `stage_incomplete` | 409 | 놀이동산 세 단계를 마치지 않고 방문 완료 요청 | 남은 단계 화면으로 |
...
> ⚠️ **본문 파싱 실패는 `fields` 가 붙지 않는다.** 어느 필드가 문제인지 서버가 알려주지 않고
> 고정 문구(`요청 본문을 읽을 수 없습니다.`)만 온다. 파서 위치와 서버 클래스명을 노출하지
> 않으려는 것이므로, 원인은 프런트가 보낸 본문을 직접 확인해야 한다.
>
> ⚠️ **`fields` 의 키는 본문과 달리 camelCase 다.** 본문은 snake_case(`variant_seed`)로
> 보내지만 검증 오류 키는 서버 필드명(`variantSeed`)이다.
